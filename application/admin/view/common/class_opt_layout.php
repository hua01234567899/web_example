...
<div class="x-body">
    ...
    <form class="layui-form" method="post">
        ...
        <div class="layui-form-item">
            ...
            <button type="button" class="layui-btn" lay-filter="post_opt" lay-submit="" id="confirm_button">
                确定
            </button>
...
<script>
    //新增分类的回调函数
    var add_s_callBack = function (res) {
        var data = res.data;
        var node = parent.currentTree;
        var new_node = {};
        new_node.id = data.class_id;
        new_node.pid = data.parent_id;
        new_node.realname = data.title;
        if (data.status == 1) {
            new_node.name = '<span>' + data.title + '</span>';
        } else {
            new_node.name = '<span class="underline_red">' + data.title + '</span>';
        }
        if (node == 'add_first_class') {
            parent.zTree.addNodes(null, new_node);
        } else {
            parent.zTree.addNodes(node, new_node);
        }
    ...
    //编辑分类的回调函数
    var edit_s_callBack = function (res) {
        var data = res.data;
        var node = parent.currentTree;
        node.realname = data.title;
        if (data.status == 1) {
            node.name = '<span>' + data.title + '</span>';
        } else {
            node.name = '<span class="underline_red">' + data.title + '</span>';
        }
    ...
    layui.use(['form', 'layer'], function () {
        ...
        form.on('submit(post_opt)', function (data) {
            ...
            if (typeof s_callBack != 'undefined' && typeof s_callBack == "function") {
                ...
                url = url.toLowerCase();
                if (url.indexOf("editclass") !== -1) {
                    var callBack = edit_s_callBack;
                } else if (url.indexOf("addclass") !== -1) {
                    var callBack = add_s_callBack;
                }
            }
